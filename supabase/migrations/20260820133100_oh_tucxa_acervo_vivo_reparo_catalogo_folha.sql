...
-- Reparo idempotente do catálogo legado e do Folha Verde.
-- Esta migration existe porque as migrations históricas 031/032 podem já constar como
-- aplicadas no Supabase. Ela reaplica os dados com seleção canônica da organização TUCXA.
...
-- Vincula as edições mensais do Folha Verde à trilha criada no Ajuste 15.
do $$
declare
  tucxa_id uuid;
  folha_trail_id uuid;
begin
  ...
  where organization_id = tucxa_id and slug = 'folha-verde-edicoes'
  limit 1;
  ...
  if folha_trail_id is not null then
    ...
    select
      ...
      100 + row_number() over (
        order by
          coalesce((resource.metadata->>'year')::integer, 0),
          coalesce((resource.metadata->>'month')::integer, 0),
          resource.title
      ),
      false,
      'Edição do Folha Verde disponível para consulta e estudo.'
    ...
    where resource.organization_id = tucxa_id
      ...
      and resource.metadata ? 'month'
    on conflict do nothing;
  end if;

  raise notice 'Acervo Vivo Ajuste 15: % título(s) e % exemplar(es) ativos na organização TUCXA.',
    (select count(*) from public.oh_acervo_titles where organization_id = tucxa_id and active = true),
    (select count(*) from public.oh_acervo_copies where organization_id = tucxa_id and active = true);
end $$;
