...
$packageName = "ae-presenca-querida-ajustes-local-foco-pendente-$timestamp"
...
Write-AE "Copiando arquivos necessários para corrigir local exibido e foco padrão da confirmação pendente..."
...
1. Trocar o local exibido na LP de "Valinhos, Campinas - SP" para "Valinhos, SP".
2. Garantir que, para todos com status "Pendente", a opção padrão visualmente marcada seja "Sim, confirma presença", não "Decidir depois".
3. Manter o status textual como "Atual: Pendente" enquanto a pessoa não registrar a resposta.
"@
