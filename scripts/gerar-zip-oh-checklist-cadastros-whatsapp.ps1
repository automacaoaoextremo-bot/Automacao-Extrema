...
$ZipName = "organizacao-em-harmonia-checklist-cadastros-whatsapp-$Timestamp.zip"
$ZipPath = Join-Path $Root $ZipName
$TempDir = Join-Path $env:TEMP "organizacao-em-harmonia-checklist-cadastros-whatsapp-$Timestamp"
...
$Readme = @"
ZIP gerado para ajustes da Organização em Harmonia.

Objetivos:
1. Corrigir WhatsApp da Organização em Harmonia para não responder como Corrente em Dia.
2. Corrigir botão Sair para redirecionar ao login da Organização em Harmonia.
3. Criar checklist inicial após login no padrão Presença Querida.
4. Criar/ajustar Base Única com cadastros similares ao Presença Querida:
   - envolvidos
   - funções
   - permissões
   - módulos habilitados
   - importação CSV
   - download de modelo
   - lista, edição, inativação e exclusão
5. Manter padrão visual:
   - desktop com menu lateral
   - mobile com menu fixo no cabeçalho
6. Atualizar documentação e roteiro de testes.
7. Manter padrão funcional validado no Corrente em Dia.
"@

Set-Content -Path (Join-Path $TempDir "_README_ZIP_OH_CHECKLIST_CADASTROS_WHATSAPP.txt") -Value $Readme -Encoding UTF8
