...
$packageName = "ae-presenca-querida-ajustes-recados-daniela-aprovacao-lp-lembretes-$timestamp"
...
Write-AE "Copiando arquivos necessários para fluxo de recados/curiosidades, aprovação, mural na LP e lembretes..."
...
$paths = @(
  ...
  "src/app/api/presenca-querida/cliente/guest-notes/route.ts",
  "src/app/api/presenca-querida/cliente/guest-notes/[id]/route.ts",
  "src/app/api/presenca-querida/public/guest-notes/[token]/route.ts",
  "src/app/solucoes/presenca-querida/cliente/recados/page.tsx",

  "src/app/api/presenca-querida/cliente/reminders/route.ts",
...
1. Melhorar o uso do campo "Curiosidade ou recado para a Daniela".
2. Enviar e-mail para a cliente/aniversariante quando chegar um novo recado.
3. Criar fluxo de aprovação antes de divulgar recados na LP.
4. Incluir uma seção na LP para exibir somente recados aprovados, com tom afetivo e persuasivo.
5. Adicionar painel logado para aprovar/reprovar/ocultar recados, se ainda não existir.
6. Atualizar próximos lembretes/mensagens para citar a novidade da LP e estimular participação.
7. Manter LGPD e privacidade:
   - não publicar recados sem aprovação;
   - permitir ocultar nome ou exibir primeiro nome;
   - não exibir telefone/observações internas;
   - permitir reprovar/remover recado.
8. Preservar o conceito Deep Dive: palavra certa, pessoa certa, objetivo específico; influência por vínculo, pertencimento e memória afetiva.
"@
