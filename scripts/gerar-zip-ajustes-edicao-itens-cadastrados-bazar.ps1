...
$ZipName = "ae-bazar-ajustes-edicao-itens-cadastrados.zip"
$TempDir = Join-Path $env:TEMP "ae-bazar-ajustes-edicao-itens-cadastrados"
...
# Inclui automaticamente SQLs extras relacionados ao Bazar/Sementinha, se existirem
$SqlDir = Join-Path $ProjectRoot "supabase/sql"

if (Test-Path $SqlDir) {
    Get-ChildItem $SqlDir -File | Where-Object {
        $_.Name -like "*bazar*" -or
        $_.Name -like "*sementinha*"
    } | ForEach-Object {
