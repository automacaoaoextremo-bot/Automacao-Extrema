...
$ZipName = "ae-funil-crm-unificado-$Timestamp.zip"
$ZipPath = Join-Path $Root $ZipName
$TempDir = Join-Path $env:TEMP "ae-funil-crm-unificado-$Timestamp"
...
$Readme = @"
ZIP gerado para ajuste do Funil / CRM unificado da Automação Extrema.

Objetivos:
1. Corrigir /admin/ae/funil para manter menu lateral da gestão AE no desktop.
2. Corrigir duplicidade de cabeçalho no Funil Corrente em Dia.
3. Criar uma única opção Funil / CRM com filtros por solução.
4. Padronizar etapas:
   - Lead recebido
   - Acesso enviado
   - Em configuração
   ...
   - Cliente ativo
   - Não convertido
5. Incluir prazo de configuração/treinamento de 30 dias para Organização em Harmonia.
6. Fazer o lead do Tucxa aparecer no CRM unificado.
7. Criar/ajustar view SQL de leads unificados, se fizer sentido.
8. Manter compatibilidade com Corrente em Dia e Organização em Harmonia.
"@

Set-Content -Path (Join-Path $TempDir "_README_ZIP_AE_FUNIL_CRM_UNIFICADO.txt") -Value $Readme -Encoding UTF8
