...
$packageName = "ae-presenca-querida-ajustes-confirmacoes-publico-mobile-$timestamp"
...
Write-AE "Copiando arquivos necessários para ajustar a tela pública de confirmações no mobile..."
...
1. Deixar a página pública de confirmações mobile friendly.
2. Corrigir overflow horizontal no celular.
3. Fazer o título quebrar corretamente e não sair da tela.
4. Reorganizar cards de totais para caberem em uma coluna no mobile.
5. Priorizar visualização rápida dos nomes das pessoas que já responderam.
6. Criar seções/listas fáceis no mobile:
   - Confirmados;
   - Talvez;
   - Pendentes;
   - Não poderão ir.
7. Melhorar tabela/lista pública para virar cards no mobile, sem scroll lateral.
8. Manter a tela somente leitura e sem ações administrativas.
9. Manter WhatsApp mascarado e ocultar dados sensíveis.
"@
