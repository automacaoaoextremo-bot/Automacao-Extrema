...
$packageName = "ae-presenca-querida-ajustes-status-confirmacoes-lembretes-$timestamp"
...
Write-AE "Copiando arquivos necessários para status pendente, painel de confirmações, cancelamento de testes e próximos lembretes..."
...
$paths = @(
  ...
  "src/app/solucoes/presenca-querida/cliente/layout.tsx",
...
1. Na LP pública, para convidados vinculados ainda não confirmados, mostrar claramente o status "Pendente" em vez de parecer confirmado.
2. Mostrar "Pendente" para qualquer convidado que ainda não confirmou.
3. Em Confirmações, criar painel completo com:
   - totais por status;
   - lista de todos os convidados;
   - status de cada convidado;
   - convidado principal/vinculado;
   - data/hora da resposta;
   - opção para cancelar/limpar respostas de teste.
4. Adicionar painel de próximos envios previstos e status.
5. Considerar planejamento do primeiro convite em 01/07/2026.
6. Programação de lembretes:
   Confirmados:
