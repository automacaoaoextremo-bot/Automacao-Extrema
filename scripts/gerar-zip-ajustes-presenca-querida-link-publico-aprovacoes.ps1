...
$packageName = "ae-presenca-querida-ajustes-link-publico-aprovacoes-$timestamp"
$tempRoot = Join-Path $env:TEMP $packageName
if (Test-Path -LiteralPath $tempRoot) { Remove-Item -LiteralPath $tempRoot -Recurse -Force }
New-Item -ItemType Directory -Path $tempRoot -Force | Out-Null

Write-AE "Preparando pacote em: $tempRoot"
Write-AE "Copiando arquivos necessários para criar link público de acompanhamento das aprovações..."
...
$paths = @(
  ...
  "src/app/solucoes/presenca-querida/acompanhamento/[slug]/page.tsx",
  "src/app/solucoes/presenca-querida/publico/[token]/page.tsx",
  "src/app/solucoes/presenca-querida/evento/[slug]/aprovacoes/page.tsx",
  "src/app/solucoes/presenca-querida/evento/[slug]/acompanhamento/page.tsx",
...
1. Criar link público para acompanhamento das aprovações sem login.
2. Link deve ser somente leitura, sem permitir aprovar/reprovar/excluir/editar.
3. Exibir resumo das aprovações dos convites personalizados:
   - total personalizado;
   - aprovados;
   - pendentes;
   - inativos;
   - novos/sem convite, se aplicável.
4. Exibir lista dos convidados/mensagens com status de aprovação.
5. Ocultar dados sensíveis no público, se possível:
   - não exibir telefone completo;
   - não exibir ações administrativas.
6. Incluir forma de copiar o link público dentro da área logada em Mensagens/Confirmações.
7. Se necessário, criar SQL para token público seguro por evento, por exemplo em pq_events.public_approval_token ou tabela similar.
8. Sugestão de URL:
