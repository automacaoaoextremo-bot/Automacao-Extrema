...
$ZipName = "organizacao-em-harmonia-agenda-viva-tucxa-regras-$Timestamp.zip"
$ZipPath = Join-Path $Root $ZipName
$TempDir = Join-Path $env:TEMP "organizacao-em-harmonia-agenda-viva-tucxa-regras-$Timestamp"
...
$Readme = @"
ZIP gerado para evolução da Organização em Harmonia com foco em Agenda Viva e regras do Tucxa.

Objetivos:
1. Corrigir o link Resolver agora do checklist Completar dados da organização.
2. Criar/ajustar tela de Cadastro da Organização.
3. Usar informações dos documentos do Tucxa no cadastro, Agenda Viva e regras iniciais.
4. Evoluir Base Única para vínculos:
   - cavalinho
...
5. Evoluir Agenda Viva para visão mobile:
   - próximos dias
   - mês
   - ano compacto similar ao calendário do Tucxa
6. Criar tipos de atividades, recorrências e aprovações.
7. Manter padrão visual do Presença Querida e Corrente em Dia.
8. Atualizar SQL, documentação e roteiro de testes.
"@

Set-Content -Path (Join-Path $TempDir "_README_ZIP_OH_AGENDA_VIVA_TUCXA_REGRAS.txt") -Value $Readme -Encoding UTF8
