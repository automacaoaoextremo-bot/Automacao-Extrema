...
function SectionLink({ link }: { link: TucxaHeaderLink }) {
  return (
    <Link
      href={link.href}
      className="inline-flex min-h-7 items-center justify-center rounded-full bg-white px-2.5 py-1 text-center text-[0.6rem] font-black uppercase tracking-[0.04em] text-[#123D2C] shadow-sm ring-1 ring-[#123D2C]/10 transition hover:-translate-y-0.5 hover:bg-[#E9F2E7] sm:min-h-10 sm:px-5 sm:py-2 sm:text-xs sm:tracking-[0.08em]"
    >
      {link.label}
    </Link>
  );
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#dfe8df] bg-white/96 shadow-sm backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-2.5 sm:px-5 sm:py-3">
        ...
        >
          <span className="relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-white p-1 shadow ring-1 ring-[#123D2C]/10 sm:h-14 sm:w-14">
            <Image src={tucxaTheme.logoSrc} alt="Logo do Tucxa" width={72} height={72} className="h-full w-full object-contain" priority />
          </span>
          <span className="min-w-0">
            <span className="block whitespace-nowrap text-[1.2rem] font-black leading-[1.05] text-[#173323] sm:text-[1.65rem]">
              {tucxaTheme.organizationName}
            </span>
            <span className="block truncate text-[0.6rem] font-black uppercase tracking-[0.18em] text-[#2F6B43] sm:text-xs sm:tracking-[0.24em]">
              {tucxaTheme.fullName}
            </span>
          </span>
        </Link>
      ...
      <div className="bg-[#fffdf7] px-4 py-1.5">
      ...
      {(actions.length > 0 || sectionLinks.length > 0) && (
        <nav className="border-t border-[#dfe8df] bg-[#F7FAF2]/95 px-2 py-1.5 sm:px-3 sm:py-2" aria-label={navLabel}>
