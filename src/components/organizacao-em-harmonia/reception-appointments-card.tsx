...
export function ReceptionAppointmentsCard() {
  const [canConsultAppointments, setCanConsultAppointments] = useState(false);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (active && response.ok) setCanConsultAppointments(payload.consultationScope ? payload.consultationScope !== "none" : payload.canReception === true || payload.canCambono === true || payload.canCavalinho === true);
    }
  ...
  if (!canConsultAppointments) return null;
  ...
  return (
    <article id="consultar-agendamentos" className="scroll-mt-44 rounded-[2rem] bg-white p-5 shadow ring-1 ring-[#123D2C]/10 transition hover:-translate-y-1 hover:shadow-xl">
      <p className="text-xs font-black uppercase tracking-[0.2em] text-[#2F6B43]">Consulta de agendamentos</p>
      <h2 className="mt-2 text-xl font-black leading-tight text-[#123D2C]">Próximos atendimentos em uma única visão.</h2>
      <p className="mt-3 text-sm font-semibold leading-6 text-slate-600">
        Pesquise por nome, WhatsApp ou entidade, acompanhe os atendimentos a partir de hoje e abra o histórico quando precisar consultar datas anteriores.
      </p>
