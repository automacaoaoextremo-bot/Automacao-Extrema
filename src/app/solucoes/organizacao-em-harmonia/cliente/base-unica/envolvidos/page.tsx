...
export default function EnvolvidosPage() {
  ...
  const authToken = useCallback(async () => {
    ...
    if (!token) router.replace("/solucoes/organizacao-em-harmonia/tucxa/filho-da-corrente");
    return token;
  }, [router]);
  ...
  async function approveAccess(person: Person) {
    ...
    try {
      ...
      if (result) setPayload(result);
      const wa = whatsappUrl(person.whatsapp, `Olá, ${person.full_name.split(/\s+/)[0] || "tudo bem"}. Seu acesso à Organização em Harmonia do Tucxa foi liberado. Acesse: ${window.location.origin}/solucoes/organizacao-em-harmonia/tucxa/filho-da-corrente`);
