...
import { consulenteAtendimentoTopics, findConsulenteAtendimentoTopic, consulenteTopicNavBySlug } from "@/lib/organizacao-em-harmonia/consulente-atendimento-content";
...
export function generateStaticParams() {
  return consulenteAtendimentoTopics.map((topic) => ({ slug: topic.slug }));
}

export default async function AtendimentoTopicConsulentePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const topic = findConsulenteAtendimentoTopic(slug);

  if (!topic) notFound();

  const nav = consulenteTopicNavBySlug[slug] ?? {
        ...
        <article className="rounded-[2rem] bg-white p-5 shadow-xl shadow-green-900/5 ring-1 ring-[#123D2C]/10 sm:p-7">
          ...
          <div className="mt-6 grid gap-4">
            {topic.sections.map((section, index) => (
              <section key={section.title} id={index === 0 ? nav.firstSectionAnchor : nav.secondSectionAnchor} className="scroll-mt-44 rounded-[1.5rem] bg-[#F7FAF2] p-5 ring-1 ring-[#123D2C]/10">
          ...
          <section id="checklist" className="mt-6 scroll-mt-44 rounded-[1.5rem] bg-white p-5 shadow ring-1 ring-[#123D2C]/10">
            ...
            <ul className="mt-4 grid gap-2 text-sm font-semibold leading-6 text-slate-700">
              {topic.checklist.map((item) => <li key={item} className="rounded-2xl bg-[#F7FAF2] p-3 ring-1 ring-[#123D2C]/10">✓ {item}</li>)}
            </ul>
          </section>
        </article>
      </section>
