...
export default async function TucxaSitePage() {
  ...
  return (
    <main id="inicio" className="min-h-screen bg-[#F7FAF2] text-[#10251C]">
      ...
      <section
        id="cadastro"
        className="scroll-mt-48 mx-auto max-w-6xl px-4 pb-6 sm:px-6 lg:px-8"
      >
      ...
      >
        <div className="grid gap-4 md:grid-cols-3">
          {[
            [
              "1",
              "A pessoa informa WhatsApp ou e-mail",
              "Se já houver cadastro, os dados podem ser conferidos pela organização. Se não houver, a pessoa preenche de forma guiada.",
            ],
            [
              "2",
              "O responsável valida",
              "A Base Única não libera automaticamente. O Tucxa confere as informações antes de conceder acesso.",
            ],
            [
              "3",
              "A orientação chega pelo canal certo",
              "Após liberar ou solicitar ajuste, o responsável pode responder por WhatsApp e por e-mail com cópia interna.",
            ],
          ].map(([step, title, text]) => (
            <article
              key={step}
              className="rounded-[1.75rem] bg-white p-5 shadow-lg shadow-green-900/5 ring-1 ring-[#123D2C]/10 sm:p-6"
            >
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-[#123D2C] text-lg font-black text-white">
                {step}
              </span>
              <h3 className="mt-4 text-lg font-black text-[#123D2C] sm:text-xl">
                {title}
              </h3>
              <p className="mt-3 text-sm leading-6 text-slate-700 sm:text-base sm:leading-7">
                {text}
