    ...
    <div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap">
      {calendarViews.map((item) => (
        <button
          ...
          className={`min-h-9 rounded-xl px-2 py-1.5 text-xs font-black shadow-sm ring-1 transition sm:px-3 sm:text-sm ${view === item.value ? "bg-[#123D2C] text-white ring-[#123D2C]" : "bg-white text-[#123D2C] ring-[#123D2C]/10"}`}
        >
          {item.label}
        </button>
      ))}
...
export default function AgendaVivaPublicaPage() {
  ...
  const modalCalendar = (
    <div className="grid gap-3">
      <div className="agenda-no-print sticky -top-3 z-30 -mx-3 -mt-3 grid gap-1.5 border-b border-[#123D2C]/10 bg-[#F7FAF2]/95 px-3 pb-2 pt-3 shadow-sm backdrop-blur sm:-top-5 sm:-mx-5 sm:-mt-5 sm:px-5 sm:pt-5">
        <div className="grid grid-cols-[2.25rem_minmax(0,0.8fr)_2.25rem_minmax(0,1fr)_minmax(0,1fr)] gap-1.5">
          <button type="button" onClick={() => move(-1)} aria-label="Período anterior" className="min-h-9 rounded-xl bg-white px-1 py-1.5 text-xs font-black text-[#123D2C] shadow ring-1 ring-[#123D2C]/10">←</button>
          <button type="button" onClick={goToday} className="min-h-9 min-w-0 rounded-xl bg-[#E9F2E7] px-1.5 py-1.5 text-xs font-black text-[#123D2C] shadow ring-1 ring-[#123D2C]/10">Hoje</button>
          <button type="button" onClick={() => move(1)} aria-label="Próximo período" className="min-h-9 rounded-xl bg-white px-1 py-1.5 text-xs font-black text-[#123D2C] shadow ring-1 ring-[#123D2C]/10">→</button>
          <button type="button" onClick={() => setFiltersOpen(true)} className="min-h-9 min-w-0 rounded-xl bg-white px-1.5 py-1.5 text-xs font-black text-[#123D2C] shadow ring-1 ring-[#123D2C]/10">Filtros</button>
          <button
            type="button"
            onClick={() => {
              setCalendarOpen(false);
            ...
            className="min-h-9 min-w-0 whitespace-nowrap rounded-xl bg-white px-1.5 py-1.5 text-xs font-black text-[#123D2C] shadow ring-1 ring-[#123D2C]/10"
          >
            <span className="sm:hidden">PDF</span>
            <span className="hidden sm:inline">Visão PDF</span>
          </button>
        </div>
  ...
  return (
    <main className="min-h-screen bg-[#F7FAF2] text-[#10251C]">
      ...
      <section id="inicio" className="mx-auto max-w-6xl scroll-mt-48 px-4 py-5 sm:px-6 lg:px-8">
        <div className="rounded-[2rem] bg-white p-4 shadow-xl shadow-green-900/5 ring-1 ring-[#123D2C]/10 sm:p-6">
          <div className="grid gap-4">
            <div>
              ...
              <p className="mt-2 text-sm font-semibold leading-6 text-slate-600">Consulte os calendários do Tucxa e, no modo Interativo, ajuste os filtros e salve o padrão neste dispositivo.</p>
            </div>
                ...
                <section className="rounded-[1.75rem] bg-[#F7FAF2] p-3 ring-1 ring-[#123D2C]/10 sm:p-4">
                  <div className="grid gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button type="button" onClick={openCalendar} className="rounded-2xl bg-[#123D2C] px-3 py-3 text-sm font-black text-white shadow ring-1 ring-[#123D2C]">Abrir Calendário</button>
