...
function whatsappLink(input: { name: string; email: string; whatsapp: string; leadId: string }) {
  ...
  const message = [
    "Olá! Preenchi o Quero Conhecer do Presença Querida e quero continuar meu cadastro pelo WhatsApp.",
    "",
    input.name ? `Nome do responsável: ${input.name}` : "",
...
export default async function PresencaQueridaObrigadoPage({
  searchParams,
}: {
    ...
    <main className="min-h-screen bg-[#fffaf8] text-slate-800">
      ...
      <section className="mx-auto max-w-3xl px-4 pb-8 pt-3 sm:pb-12 sm:pt-6">
        <div className="rounded-[2rem] bg-white p-5 shadow-xl ring-1 ring-rose-100 sm:p-8">
          <p className="text-xs font-black uppercase tracking-[0.28em] text-[#E85D75] sm:text-sm">Cadastro recebido</p>
          ...
          <p className="mt-3 text-base leading-7 text-slate-700 sm:text-lg sm:leading-8">
            Você já iniciou o processo para organizar convidados, confirmações e mensagens com mais carinho e previsibilidade. Agora continue pelo WhatsApp da Automação Extrema para receber as orientações de acesso e iniciar a avaliação como Cliente Fundador.
          </p>

          <div className="mt-4 rounded-3xl bg-rose-50 p-5 text-slate-800">
          ...
          <div className="mt-5">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex min-h-14 w-full items-center justify-center rounded-2xl bg-[#E85D75] px-6 py-4 text-center text-base font-black text-white shadow-lg shadow-rose-900/15 transition hover:-translate-y-0.5 hover:bg-[#f06c84]"
            >
