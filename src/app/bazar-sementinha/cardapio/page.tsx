      ...
      <main className="min-h-screen bg-[#f9f7ef] text-[#214527]">
        <section className="border-b border-[#dfe8df] px-4 py-10 sm:py-14">
          <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1fr_0.86fr] lg:items-center">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.2em] text-[#83a847]">Cardápio público · Bazar do Sementinha · 04/07</p>
              <h1 className="mt-4 text-4xl font-black leading-tight sm:text-6xl">Cozinha do Bazar Sementinha</h1>
              <p className="mt-5 max-w-2xl text-lg leading-8 text-[#31543a]">
                Veja fotos, vídeo e valores antes de chamar a equipe. No dia do bazar, o pedido é registrado no sistema, o pagamento passa pelo caixa e você pode acompanhar o pedido pelo QRCode.
              </p>
              <div className="mt-6 flex flex-wrap gap-3">
                <Link href="/bazar-sementinha/pedidos" className="rounded-full bg-[#2f7d45] px-6 py-3 text-sm font-black uppercase tracking-[0.12em] text-white shadow-sm">
                  Chamar equipe para pedido
                </Link>
                <a href="#itens-cardapio" className="rounded-full border border-[#2f7d45]/20 bg-white px-6 py-3 text-sm font-black uppercase tracking-[0.12em] text-[#2f7d45] shadow-sm">
                  Ver itens e valores
                </a>
              </div>
            </div>
            <aside className="rounded-[2rem] border border-[#dfe8df] bg-white p-4 shadow-sm">
              <Image src={POSTER_SRC} alt="Cardápio Cozinha do Bazar Sementinha 04/07" width={900} height={1600} priority className="mx-auto max-h-[680px] w-auto rounded-[1.5rem] object-contain" />
            </aside>
          </div>
        </section>
        ...
        <section className="px-4 py-10">
          <div className="mx-auto grid max-w-6xl gap-5 md:grid-cols-3">
            <article className="rounded-3xl border border-[#dfe8df] bg-white p-5 shadow-sm">
              <h2 className="text-xl font-black">Escolha sem pressa</h2>
              <p className="mt-3 leading-7 text-[#496451]">Veja preços e opções antes de pedir para evitar dúvidas na fila.</p>
            </article>
            <article className="rounded-3xl border border-[#dfe8df] bg-white p-5 shadow-sm">
              <h2 className="text-xl font-black">Pedido com QRCode</h2>
              <p className="mt-3 leading-7 text-[#496451]">Após a criação, o cliente pode abrir o QRCode e acompanhar total, itens e status.</p>
            </article>
            <article className="rounded-3xl border border-[#dfe8df] bg-white p-5 shadow-sm">
              <h2 className="text-xl font-black">Caixa organizado</h2>
              <p className="mt-3 leading-7 text-[#496451]">A equipe registra, o caixa confere e a prestação de contas fica mais clara.</p>
            </article>
          </div>
        </section>

        <section className="border-y border-[#dfe8df] bg-white px-4 py-10">
          <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.2em] text-[#83a847]">Vídeo simples para TV e divulgação</p>
              <h2 className="mt-3 text-3xl font-black sm:text-4xl">Cardápio em vídeo para rodar no evento</h2>
              <p className="mt-4 leading-8 text-[#496451]">
        ...
        <section id="itens-cardapio" className="px-4 py-10 sm:py-14">
          <div className="mx-auto max-w-6xl">
            <div className="flex flex-wrap gap-2">
              <a href="#todos" className="rounded-full bg-[#006b35] px-5 py-3 text-sm font-black text-white">Todos</a>
              {categories.map((category) => (
                <a key={category} href={`#${slugify(category)}`} className="rounded-full bg-white px-5 py-3 text-sm font-black text-[#214527] shadow-sm ring-1 ring-[#dfe8df]">
                  {category}
                </a>
              ))}
            </div>
            ...
            <div id="todos" className="mt-8 space-y-10">
              {groupedItems.map((group) => (
                <section key={group.category} id={slugify(group.category)} className="scroll-mt-44 rounded-[2rem] border border-[#dfe8df] bg-white p-4 shadow-sm sm:p-6">
                  <div className="grid gap-6 lg:grid-cols-[360px_1fr] lg:items-start">
                    <div className="overflow-hidden rounded-[1.5rem] bg-[#f9f7ef]">
                      <Image src={categoryPhoto(group.category)} alt={`Foto ilustrativa da categoria ${group.category}`} width={1200} height={675} className="h-64 w-full object-cover lg:h-full" />
                    </div>
                    <div>
                      <p className="text-sm font-black uppercase tracking-[0.18em] text-[#83a847]">{group.category}</p>
                      <h2 className="mt-2 text-3xl font-black">{categoryTitle(group.category)}</h2>
                      <p className="mt-3 leading-7 text-[#496451]">{categoryIntro(group.category)}</p>
                      <div className="mt-5 grid gap-4 sm:grid-cols-2">
                        {group.items.map((item) => (
                          <article key={item.id} className="rounded-3xl border border-[#dfe8df] bg-[#fffdf7] p-5 shadow-sm">
                            ...
                            <strong className="mt-4 block text-3xl text-[#0f6b35]">{formatBRL(item.price)}</strong>
