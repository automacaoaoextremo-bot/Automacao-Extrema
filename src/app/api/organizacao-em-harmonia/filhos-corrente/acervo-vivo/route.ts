...
export async function GET(request: Request) {
  ...
  if (access.context.profile !== "filho-da-corrente") {
    return NextResponse.json({ error: "Este acesso pertence ao painel do Filho da Corrente." }, { status: 403 });
  }
...
export async function POST(request: Request) {
  return handleAcervoReaderPost(request, "filho-da-corrente");
}
