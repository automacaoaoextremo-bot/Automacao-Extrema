...
async function currentConsulente(request: Request, organizationId: string) {
  const token = bearerToken(request);
  if (!token) throw new Error("Sessão expirada. Entre novamente no painel do Filho de Fora/Consulente.");

  const { data: userData, error: userError } = await supabaseAdmin.auth.getUser(token);
  if (userError || !userData.user) throw new Error("Sessão inválida. Entre novamente no painel do Filho de Fora/Consulente.");

  const user = userData.user;
  const email = user.email || "";
  const userMetadata = asRecord(user.user_metadata);
  const userProfile = normalize(asText(userMetadata.oh_profile));
  ...
  if (!person) throw new Error("Cadastro do Filho de Fora/Consulente não localizado.");
  ...
  const agendaProfile = asRecord(membership.agenda_viva_profile);
  const accessType = normalize(asText(agendaProfile.accessType) || asText(agendaProfile.publico));
  const profileIsConsulente = ["consulente", "filho-de-fora", "filho de fora"].includes(userProfile);
  const membershipIsConsulente = accessType.includes("consulente") || accessType.includes("filho-de-fora") || accessType.includes("filho de fora");

  if (!profileIsConsulente && !membershipIsConsulente) {
    throw new Error("Este acesso não pertence ao painel do Filho de Fora/Consulente.");
  }
...
function rawEventAudience(event: EventRecord) {
  const metadata = asRecord(event.metadata);
...
export async function GET(request: Request) {
  try {
    ...
    const message = error instanceof Error ? error.message : "Erro ao carregar Agenda Viva do Filho de Fora/Consulente.";
