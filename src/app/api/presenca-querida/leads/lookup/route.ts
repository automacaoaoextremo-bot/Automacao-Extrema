...
import { PRESENCA_LEAD_STATUS_LABELS, type PresencaLeadStatus } from "@/lib/presenca-querida";
...
const LEAD_SELECT = "id, responsible_name, event_name, email, whatsapp, status, access_sent_at, access_user_email, created_at";
...
function buildPhoneCandidates(rawWhatsapp: string | null | undefined): PhoneCandidates {
  ...
  if (!digits || isBotconversaLiteral(raw)) {
    return { raw, digits: "", withoutBrazilCode: "", last11: "", candidates: [] };
  }
  ...
  const candidates = unique([digits, withoutBrazilCode, last11, last10, `55${withoutBrazilCode}`, `55${last11}`, `55${last10}`]);
...
function buildFallbackMessage(loginUrl: string) {
  return [
    ...
    "",
    "As orientações de acesso foram preparadas para o e-mail informado no formulário do Presença Querida.",
    "",
    "Acesse:",
    loginUrl,
    "",
...
function buildSuccessMessage(input: { name: string | null; eventName: string | null; loginUrl: string; accessEmail: string | null }) {
  const greeting = firstName(input.name);

  return [
    `Pronto, ${greeting}. Localizei seu cadastro no Presença Querida.`,
    "",
    input.eventName ? `Evento: ${input.eventName}` : "",
    "Seu acesso inicial já foi preparado para você começar a configuração do evento.",
    "",
    "Link de acesso:",
    input.loginUrl,
    "",
    ...
    "",
    "Próximo passo:",
    "entre no sistema, complete o cadastro do evento, ajuste convite, cadastre convidados e teste uma confirmação.",
    "",
...
function responsePayload(input: {
  ...
  const status = (input.lead?.status ?? (input.found ? "localizado" : "cadastro_nao_localizado_whatsapp")) as PresencaLeadStatus;
  ...
  return {
    ...
    eventName: input.lead?.event_name ?? null,
    email: input.lead?.email ?? null,
    whatsapp: input.lead?.whatsapp ?? null,
    status,
    statusLabel: PRESENCA_LEAD_STATUS_LABELS[status] ?? status,
...
async function findLeadByLeadId(leadId: string) {
  ...
  const { data, error } = await supabaseAdmin.from("pq_leads").select(LEAD_SELECT).eq("id", leadId).maybeSingle();
...
async function findLeadByWhatsapp(rawWhatsapp: string | null | undefined) {
  ...
  if (phone.candidates.length === 0) return null;
...
export async function GET() {
  ...
  return NextResponse.json({
    ok: true,
    service: "presenca-querida-leads-lookup",
    method: "POST",
    expectedBody: {
      whatsapp: "{telefone}",
      leadId: "opcional",
    ...
    loginUrl: `${baseUrl}/solucoes/presenca-querida/login`,
  });
}
...
export async function POST(request: Request) {
  ...
  const baseUrl = siteUrl();
  const leadFormUrl = `${baseUrl}/solucoes/presenca-querida/quero-conhecer`;
  ...
  try {
    ...
    const botconversaMessage = buildSuccessMessage({ name: lead.responsible_name, eventName: lead.event_name, loginUrl, accessEmail });
